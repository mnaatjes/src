...
class AriaController {
  ...
  get enabled(){
    ...
    let acc = [];
    /**
     * loop aria properties and populate array
     */
    this.#defaultAttributes.forEach(attribute => {
      let property = this[attribute.propertyName];
      if(property.enabled === true){
        acc.push(attribute.propertyName);
      }
    });
    /**
     * determine length of accumulator
     */
    if(acc.length > 0){
      return acc;
    } else {
...
class AriaAttribute {
  ...
  remove(){
    /**
     * remove attribute from html tag
     */
